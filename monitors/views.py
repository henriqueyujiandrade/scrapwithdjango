...
from .serializers import MonitorSerializer, MonitorGetSerializer, MonitorDetailSerializer, MonitorPatchSerializer
from rest_framework import generics
...
class MonitorScrappingView(APIView):  
  ...
  def patch(self, request: Request) -> Response:
        ...
        for monitor in monitors:
          ...
          if monitor_data:
            serializer = MonitorPatchSerializer(monitor_data[0], data={'current_price': monitor['current_price']}, partial=True)
            serializer.is_valid(raise_exception=True)
